/*
  The following code implements the close loop velocity with PID controller and potentiometer. we have a similar implementation to the one with velocity, only this time we took the 
  delta of positions and by dividing it by the delta of time we manage to control the speed. we used our implementations in the previous sections regarding the motor, RPM and speed
  to reach this result. In addition, several variables of the controller and filtering were chose based on different measurements and results (several to see which is effective)
  as described in the code. This time the user doesn't need to enter a string, he needs to move the potentiometer in order to determine the desired speed.
*/
// Define Pins
...
const double maxPos = 255.0;
const double minPos = -255.0;
double lastTime = 0.0;
double curr_loc = 0.0;
double prev_loc = 0.0;
...
void loop() {
 ...
 // }
    int desired_speed = analogRead(A0);
    desired_speed = map(desired_speed, 0, 1023, -200, 200); //where 200 is the maximal RPM by our calculations for each side
    ...
    RPM = constrain(RPM, minPos, maxPos); //for the limits declared at top
    if (RPM < 0) {
